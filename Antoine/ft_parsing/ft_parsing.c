...
/*   Updated: 2023/07/07 11:27:12 by anvincen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parsing(t_data *data)
{
	...
	tab.tab_position = (int *)malloc(ft_strlen(data->str) * sizeof(int));
	if (!tab.tab_position)
		return ;
	i = 0;
	tab.nb_cmd = 0;
	...
	while (data->str[quote.i] != 0)
	{
		...
		i = ft_without_quotes2(&quote, &tab, data->str, i);
		quote.i++;
	}
	if (data->str[0] != '|'
		&& data->str[0] != '<'
		&& data->str[0] != '>')
		tab.nb_cmd++;
	ft_parsing2(&tab, data->str);
	free(data->str);
	data->tab_cmd = tab.tab_cmd;
	data->nb_cmd = tab.nb_cmd;
	ft_parsing6(data);
}
